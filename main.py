...
CHANNEL_ID = '@kinoru_kgz'  # Сиздин каналдын ID'си
...
def start(message):
    ...
    btn = types.InlineKeyboardButton("📢 Каналга катталуу", url="https://t.me/kinoru_kgz")
    markup.add(btn)
...
def handle_search(message):
    ...
    # 1. Каналга катталганын текшерүү
    if not is_subscribed(user_id):
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("📢 Каналга катталуу", url="https://t.me/kinoru_kgz"))
        bot.reply_to(message, "⚠️ Издөө үчүн алгач биздин каналга катталыңыз:", reply_markup=markup)
        return
    ...
    # "Издөөдө..." билдирүүсү (тегеренүү эффектиси үчүн)
    msg = bot.send_message(message.chat.id, "⏳ Издөөдө...")
    
    # Анимация сыяктуу жазууну алмаштыруу
    time.sleep(0.5)
    ...
    # YouTube издөө шилтемеси
    link = f"https://www.youtube.com/results?search_query={search_query.replace(' ', '+')}"
    
    # "Музыка" баскычы (сиз сурагандай орусча)
    btn_music = types.InlineKeyboardButton("🎵 Музыка", url=link)
